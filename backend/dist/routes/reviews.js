...
router.get('/rubrics/:competitionId', auth, async (req, res) => {
    try {
...
router.put('/rubrics/:competitionId', auth, async (req, res) => {
    try {
        const { rubrics } = req.body;
        // 删除旧的
        await prisma.scoringRubric.deleteMany({ where: { competitionId: req.params.competitionId } });
        // 创建新的
...
router.get('/reviewers/:competitionId', auth, async (req, res) => {
    try {
...
router.put('/reviewers/:competitionId', auth, async (req, res) => {
    try {
        const { teacherIds } = req.body;
        const now = new Date().toISOString();
        // 删除旧指派
        await prisma.reviewerAssignment.deleteMany({ where: { competitionId: req.params.competitionId } });
        // 创建新指派
        for (const tid of teacherIds) {
            ...
            // 发送通知
            const comp = await prisma.competition.findUnique({ where: { competitionId: req.params.competitionId } });
...
router.get('/scores/:registrationId', auth, async (req, res) => {
    try {
...
router.get('/my-score/:registrationId', auth, async (req, res) => {
    try {
...
router.get('/progress/:competitionId', auth, async (req, res) => {
    try {
...
router.get('/average/:registrationId', auth, async (req, res) => {
    try {
