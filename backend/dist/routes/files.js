...
import fs from 'fs';
const router = Router();
async function canAccessRegistration(registrationId, userId, role) {
    if (role === 'admin')
        return true;
    ...
    if (!reg)
        return false;
    if (role === 'teacher')
        return reg.competition.creatorId === userId;
...
router.post('/:registrationId', auth, upload.array('files', 10), async (req, res) => {
    try {
        const registrationId = req.params.registrationId;
        const reg = await prisma.registration.findUnique({
            where: { registrationId },
